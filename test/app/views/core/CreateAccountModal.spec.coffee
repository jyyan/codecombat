...
#    jasmine.demoModal(modal)
  ...
  describe 'BasicInfoView', ->
    ...
    describe 'checkNameExists()', ->
      beforeEach ->
        basicInfoView.$('input[name="name"]').val('Some Name')
        basicInfoView.checkNameExists()

      it 'shows checking', ->
        expect(basicInfoView.$('[data-i18n="signup.checking"]').length).toBe(1)

      # TODO: Finish setting up name checker
...
#          done()
    ...
    describe 'onSubmitForm()', ->
      ...
      describe 'when using a password', ->
        it 'checks for conflicts, updates user, and signs them up', (done) ->
          ...
          nameConflictRequest = jasmine.Ajax.requests.mostRecent()
          nameConflictRequest.respondWith({
            status: 200
            responseText: JSON.stringify({
              conflicts: false
              givenName: 'A Username'
              suggestedName: 'A Username'
            })
          })
          
          _.defer ->
            saveUserRequest = jasmine.Ajax.requests.mostRecent()
            saveUserRequest.respondWith({
              status: 200
              responseText: '{}'
            })
            
            _.defer ->
              signupUserRequest = jasmine.Ajax.requests.mostRecent()
              signupUserRequest.respondWith({
                status: 200
                responseText: '{}'
              })
              done()
